// EDtunnel - A Cloudflare Worker-based vll pxx with WebSocket Transport
// @ts-ignore
 ...
 */
let userID = 'c865523a-7fbf-467c-a477-31d6679b8a42';

/**
 * Array of pxx server addresses with ports
 * Format: ['hostname:port', 'hostname:port']
 */
const pxxIPs = [];

// Randomly select a pxx server from the pool
let pxxIP = pxxIPs[Math.floor(Math.random() * pxxIPs.length)];
let pxxPort = pxxIP && pxxIP.includes(':') ? pxxIP.split(':')[1] : '443';
 ...
 * @param {string} env.pxxIP - pxx server IP address
 * @param {string} env.skk5 - skk5 pxx configuration
 * @param {string} env.skk5_RELAY - skk5 relay mode flag
...
export default {
	...
	async fetch(request, env, _ctx) {
		try {
			const { UUID, pxxIP, skk5, skk5_RELAY } = env;
			...
			// 记录参数值，用于调试
			console.log('配置参数:', requestConfig.userID, requestConfig.skk5Address, requestConfig.skk5Relay, urlpxxIP);

			// Handle pxx configuration for the current request
			const pxxConfig = handlepxxConfig(urlpxxIP || pxxIP);
			...
			if (request.headers.get('Upgrade') !== 'websocket') {
				...
				if (matchingUserID) {
					if (url.pathname === `/${matchingUserID}` || url.pathname === `/sub/${matchingUserID}`) {
						...
						const pxxAddresses = pxxIP ? pxxIP.split(',').map(addr => addr.trim()) : requestConfig.pxxIP;
						const content =
							getConfig(matchingUserID, host, pxxAddresses);
...
/**
 * Handles outbound TCP connections for the pxx.
...
async function HandleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, protocolResponseHeader, log, config = null) {
	...
	async function connectAndWrite(address, port, skk = false) {
		...
		let tcpSocket;
		if (config.skk5Relay) {
			tcpSocket = await skk5Connect(addressType, address, port, log, config.parsedskk5Address)
		} else {
			tcpSocket = skk ? await skk5Connect(addressType, address, port, log, config.parsedskk5Address)
				: connect({
					hostname: address,
					port: port,
				});
	...
	async function retry() {
		...
		if (config.enableskk) {
			...
			tcpSocket = await connectAndWrite(config.pxxIP || addressRemote, config.pxxPort || portRemote, false);
		}
...
function ProcessProtocolHeader(protocolBuffer, userID) {
	...
	const uuids = (userID && userID.includes(',')) ? userID.split(",") : [userID];
...
async function skk5Connect(addressType, addressRemote, portRemote, log, parsedskk5Addr = null) {
	// 如果没有传入解析的skk5地址，使用全局的
	const { username, password, hostname, port } = parsedskk5Addr || parsedskk5Address;

	// Connect to the skk server
	const socket = connect({
		hostname,
		port,
	});

	// Request head format (Worker -> skk Server):
	// +----+----------+----------+
	...
	// https://en.wikipedia.org/wiki/skk#skk5
	// For METHODS:
	...
	// Response format (skk Server -> Worker):
	// +----+--------+
	// |VER | METHOD |
	// +----+--------+
	// | 1  |   1    |
	...
	// Request data format (Worker -> skk Server):
	// +----+-----+-------+------+----------+----------+
	...
	const skkRequest = new Uint8Array([5, 1, 0, ...DSTADDR, portRemote >> 8, portRemote & 0xff]);
...
/**
 * Parses skk5 address string.
 * @param {string} address - skk5 address string
...
/**
 * Generates configuration for vll client.
 * @param {string} userIDs - Single or comma-separated user IDs
 * @param {string} hostName - Host name for configuration
 * @param {string|string[]} pxxIP - pxx IP address or array of addresses
 * @returns {string} Configuration HTML
 */
function getConfig(userIDs, hostName, pxxIP) {
  ...
  `;
    ...
    <div class="container">
      ...
      <p>Welcome! This function generates configuration for the vll protocol. If you found this useful, please check our GitHub project:</p>
      ...
      <div class="btn-group">
        <a href="//${hostName}/sub/${userIDArray[0]}" class="btn" target="_blank"><i class="fas fa-link"></i> vll Subscription</a>
      ...
      <div class="subscription-info">
        ...
        <ul>
          <li><strong>vll Subscription:</strong> Direct link for vll protocol configuration. Suitable for clients supporting vll.</li>
          ...
          <li><strong>Clash Link:</strong> A web link to convert the vll config to Clash format. Useful for manual import or troubleshooting.</li>
          <li><strong>Best IP Subscription:</strong> Provides a curated list of optimal server IPs for many <b>different countries</b>.</li>
        </ul>
        <p>Choose the option that best fits your client and needs. For most users, the vll or Clash Subscription will be the easiest to use.</p>
      </div>
    </div>
  `;
	...
	const configOutput = userIDArray.map((userID) => {
		...
		const protocolSec = atob(pt) + '://' + userID + atob(at) + (pxxIP && pxxIP[0] ? pxxIP[0].split(':')[0] : '') + ":" + pxxPort + commonUrlPart;
		return `
      <div class="container config-item">
        ...
        <div class="input-group mb-3">
          <select class="form-select" id="pxxSelect" onchange="updatepxxConfig()">
            ${typeof pxxIP === 'string' ?
				`<option value="${pxxIP}">${pxxIP}</option>` :
				Array.from(pxxIP).map(pxx => `<option value="${pxx}">${pxx}</option>`).join('')}
          </select>
        </div>
		<br>
        <div class="code-container">
          <pre><code id="pxxConfig">${protocolSec}</code></pre>
          <button class="btn copy-btn" onclick='copyToClipboard(document.getElementById("pxxConfig").textContent)'><i class="fas fa-copy"></i> Copy</button>
        </div>
      </div>
    `;
	}).join('');
  ...
  <body>
    ...
    <script>
      ...
      function updatepxxConfig() {
        const select = document.getElementById('pxxSelect');
        const pxxValue = select.value;
        const [host, port] = pxxValue.split(':');
        const protocolSec = atob(pt) + '://' + userIDArray[0] + atob(at) + host + ":" + port + commonUrlPart;
        document.getElementById("pxxConfig").textContent = protocolSec;
      }
    </script>
  </body>
  </html>`;
 ...
 */
function GenSub(userID_path, hostname, pxxIP) {
	...
	const pxxIPArray = Array.isArray(pxxIP) ? pxxIP : (pxxIP ? (pxxIP.includes(',') ? pxxIP.split(',') : [pxxIP]) : pxxIPs);
	...
	const result = userIDArray.flatMap((userID) => {
		...
		// Generate pxx HTTPS URLs
		pxxIPArray.forEach((pxxAddr) => {
			const [pxxHost, pxxPort = '443'] = pxxAddr.split(':');
			const urlPart = `${hostname.split('.')[0]}-${pxxHost}-HTTPS-${pxxPort}`;
			const secondaryProtocolHttps = atob(pt) + '://' + userID + atob(at) + pxxHost + ':' + pxxPort + commonUrlPartHttps + urlPart + '-' + atob(ed);
...
/**
 * Handles pxx configuration and returns standardized pxx settings
 * @param {string} pxxIP - pxx IP configuration from environment
 * @returns {{ip: string, port: string}} Standardized pxx configuration
 */
function handlepxxConfig(pxxIP) {
	if (pxxIP) {
		const pxxAddresses = pxxIP.split(',').map(addr => addr.trim());
		const selectedpxx = selectRandomAddress(pxxAddresses);
		const [ip, port = '443'] = selectedpxx.split(':');
		return { ip, port };
	} else {
		const port = pxxIP && pxxIP.includes(':') ? pxxIP.split(':')[1] : '443';
		const ip = pxxIP && pxxIP.includes(':') ? pxxIP.split(':')[0] : '';
		return { ip, port };
	}
}
